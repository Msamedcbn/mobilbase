                  ...
                  <tbody>
                    {users.map((u) => {
                      ...
                      const roleColors: Record<string, { bg: string; text: string; label: string }> = {
                        ...
                        MANAGER: { bg: "bg-blue-500/10", text: "text-blue-600", label: "Müdür" },
                        ACCOUNTANT: { bg: "bg-slate-100", text: "text-slate-600", label: "Muhasebeci" },
                        TECHNICIAN: { bg: "bg-slate-100", text: "text-slate-600", label: "Teknisyen" },
                        CASHIER: { bg: "bg-slate-100", text: "text-slate-600", label: "Kasiyer" },
                      ...
                      return (
                        <tr key={u.id} className="hover:bg-slate-50/50">
                          <td>
                            <div className="flex items-center gap-3">
                              <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-blue-400 to-blue-500 text-white font-bold text-xs flex items-center justify-center shadow-sm shadow-blue-900/20 shrink-0 select-none">
                                {initials}
                              </div>
                            ...
                            {(() => {
                              ...
                              return (
                                <div className="flex flex-col items-end">
                                  <span className="font-bold font-mono text-sm text-slate-700">{today.revenue.toLocaleString("tr-TR")} TL</span>
                                  <span className="text-2xs text-slate-400"><span className="font-mono">{today.salesCount}</span> satış</span>
                                </div>
                              );
                            })()}
                          </td>
                          <td>
                            {cari ? (
                              ...
                              >
                                <span className={`font-mono ${cari.balance > 0 ? "text-rose-500" : cari.balance < 0 ? "text-emerald-500" : "text-slate-400"}`}>
              ...
              <div className="inline-flex rounded-2xl border border-slate-200 bg-white p-1 text-xs font-bold shadow-sm">
                {(["day", "week", "month", "all"] as const).map((p) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl transition-all cursor-pointer ${!useCustomRange && performancePeriod === p ? "bg-blue-600 text-white shadow-sm shadow-blue-900/20" : "text-slate-600 hover:bg-slate-100"}`}
                  >
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {staffPerformance.map((row) => (
                      <tr key={row.userId} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-right font-mono text-slate-600">{row.salesCount}</td>
          ...
          <div className="w-full max-w-[680px] bg-white border border-slate-200 rounded-2xl shadow-2xl p-5 space-y-3 animate-scale-in">
            <div className="flex justify-between items-start">
              ...
              <button
                ...
                className="w-8 h-8 shrink-0 rounded-full border border-slate-200 flex items-center justify-center hover:bg-slate-50 text-slate-500 cursor-pointer transition-colors"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    ...
                    {userBenefits.map((b, idx) => (
                      <div key={idx} className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="shrink-0 w-8 h-8 rounded-lg border border-slate-200 text-rose-500 hover:bg-rose-50 transition-colors flex items-center justify-center"
                        >
                          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
                          </svg>
