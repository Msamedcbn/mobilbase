...
## Project Context
- Repository is a Next.js 14 app named `vibegsm`.
- Public homepage lives in `app/page.tsx`.
- Global styling lives in `app/globals.css`; Tailwind scans `app/**/*` and `components/**/*`.
- Root layout metadata currently says `VibeGSM | Telefon Bayi Otomasyonu`.
...
- Page copy positions VibeGSM as a cloud platform for phone retail, service, stock, finance, POS, and buyback operations.
...
## Clarification
- Primary homepage goal: introduce VibeGSM and convince phone bayis that this is the system they need.
